...
for (y in years) {
  for (m in months) {
    file_name <- file.path("Bases de Datos", "Clima", "Lluvia", paste(paste(y, m, sep="-"), "csv", sep="."))
    data <- read.csv(file_name)[c(1:2, 6)]
    names(data) <- c("longitud", "latitud", "lluvia")
    assign(paste("lluvia", y, m, sep="_"), data)
  }
}
...
# Objeto con nombres de lluvia:
lluvia <- as.character()
for (y in years) {
  for (m in months) {
    lluvia <- c(lluvia, paste("lluvia", y, m, sep="_"))
  }
}
...
for (i in municipios) {
  ...
  for (y in years) {
    for (m in months) {
      ...
      clima <- get(paste("lluvia", y, m, sep="_"))
...
write.xlsx(base_final, "Bases de Datos/Finales/Lluvia.xlsx")
